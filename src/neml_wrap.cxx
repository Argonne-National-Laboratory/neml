...
PYBIND11_PLUGIN(neml) {
  ...
  py::class_<SmallStrainViscoPlasticity, std::shared_ptr<SmallStrainViscoPlasticity>>(m, "SmallStrainViscoPlasticity", py::base<NEMLModel_sd>())
      ...
      .def("set_trial_state",
           [](SmallStrainViscoPlasticity & m, py::array_t<double, py::array::c_style> e_np1, py::array_t<double, py::array::c_style> h_n, double T_np1, double t_np1, double t_n) -> void
           {
              int ier = m.set_trial_state(arr2ptr<double>(e_np1), arr2ptr<double>(h_n), T_np1, t_np1, t_n);
              py_error(ier);
           }, "Setup trial state for solve.")
      ...
      .def("RJ",
           [](SmallStrainViscoPlasticity & m, py::array_t<double, py::array::c_style> x) -> std::tuple<py::array_t<double>, py::array_t<double>>
           {
            auto R = alloc_vec<double>(m.nparams());
            auto J = alloc_mat<double>(m.nparams(), m.nparams());
