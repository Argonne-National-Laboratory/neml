...
WorkRateFunctionDamage_sd::WorkRateFunctionDamage_sd(
    ...
    bool verbose, bool truesdell) :
      NEMLScalarDamagedModel_sd(elastic, base, alpha, tol, miter, verbose, truesdell,false, 0, 1),
      workrate_(workrate), Q_(Q), m_(m), G_(G), H_(H), xi_(xi), phi_(phi)
{
    ...
    double * const dd) const
{
  ...
  double denominator  = workrate_->value(work_rate);
  double ddenominator = workrate_->derivative(work_rate);

  double firstTerm  = m * pow(work_rate/Q,m-1) / (Q * denominator);
  double secondTerm = pow(work_rate/Q,m) * -1 * ddenominator / pow(denominator,2);
    ...
    double * const dd) const
{
  ...
  double fval = (firstTerm + secondTerm );
