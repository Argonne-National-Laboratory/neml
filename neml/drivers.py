...
def uniaxial_test(model, erate, T = 300.0, T_ramp = None, emax = 0.05, nsteps = 250,
  ...
  """
    ...
    Keyword Args:
      ...
      T_ramp:           max temperature to ramp to during the test, default is no ramp
  ...
  temperature_history = np.linspace(T, Tmax, nsteps)
  ...
  for i in range(nsteps):
    try:
      if i == 0:
        einc, ainc = driver.erate_einc_step(sdir, erate, e_inc, temperature_history[i])
      else:
        einc, ainc = driver.erate_einc_step(sdir, erate, e_inc, temperature_history[i],
